...
Notice that this satisfy the condition in @existence-of-potential-function, we can conclude that there exists a function $S: U -> bb(R)$ such that:

$
  pdv(S, u_j) = g compose vb(phi) quad "and" quad pdv(S, u_i) = psi
$

Then, we can rewrite the first equation as:

$
  pdv(S, u_j) & = g(u_1, dots.c, u_(j-1), psi, u_(j+1), dots.c) \
  pdv(S, u_j) & = g(u_1, dots.c, u_(j-1), pdv(S, u_i), u_(j+1), dots.c)
$

Notice that $u_j$ does not appear explicitly on the RHS. Therefore, consider separating the variables, we can assume a solution of the form:

$ S(vb(u)) = alpha u_j + S'(u_1, dots.c, u_(j-1), u_(j+1), dots.c) $

Therefore,

$ pdv(S, u_j) = alpha quad "and" quad pdv(S, u_i) = pdv(S', u_i) $

Plugging them back to the equation, we have:

$ alpha = g(u_1, dots.c, u_(j-1), pdv(S', u_i), u_(j+1), dots.c) $

Recall that the goal is to solve for $psi$:

$ psi = pdv(S, u_i) = pdv(S', u_i) $

Substituting $psi$ back in, we have:

$ alpha = g(u_1, dots.c, u_(j-1), psi, u_(j+1), dots.c) $

Since $g$ is invertible with respect to dimension $j$, we can solve for $psi$:

$ psi = g^(-1)(alpha; u_1, dots.c, u_(j-1), u_(j+1), dots.c) $

where $g^(-1)$ is the inverse function of $g$ with respect to dimension $j$.

Putting it back to the transformation, we have:

$
  vb(phi)(vb(u)) = sum_(i != j) u_i vb(e_i) + g^(-1)(alpha; u_1, dots.c, u_(j-1), u_(j+1), dots.c) dot vb(e_j)
$

This explicit form of $vb(phi)$ satisfies the properties we need, and can be used to simplify the integral as shown above.

Let's write the final result as a theorem:

#theorem[
  Given a function $f: bb(R) -> bb(R)$, whose anti-derivative $F$ is known, and another function $g: bb(R)^n -> bb(R)$, whose invertible with respect to at least one variable $u_j$. Let $D$ be an open set in $bb(R)^n$. Suppose we have the following integral:

  $ integral_D ( f compose g )(vb(v)) dd(vb(v)) $

  The transformation $vb(phi)$ defined as:
  ...
  where $U$ is an open set in $bb(R)^n$, such that $vb(phi)(U) = D$.
]
